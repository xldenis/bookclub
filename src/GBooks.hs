...
data Book = Book { title :: Text, author :: Text, description :: Text, isbn :: Text } deriving (Show, Eq)
...
toBook value = Book
  ...
  <*> value ^? key "industryIdentifiers" . _Array . to (V.filter $ \o -> o ^?! key "type" == "ISBN_13") . ix 0 . key "identifier" . _String
